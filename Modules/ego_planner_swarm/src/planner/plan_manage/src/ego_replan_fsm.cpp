...
namespace ego_planner
{
  ...
  void EGOReplanFSM::generateWps()
  {
    ...
    {
      waypoint_num_ = generateSinWaypoints(box_min_x, box_max_x, box_min_y, box_max_y, 
                      box_min_z, box_max_z, stepXY_);
    }
    else if (waypointDistriFlag_ == 2)
    {
      waypoint_num_ =  generateSpiralWaypoints(coordinate_X_, coordinate_Y_, radius_R_, 
                      numSpiralSegments_, minZ_rotate_, maxZ_rotate_, stepZ_rotate_);
    } else {
  ...
  int EGOReplanFSM::generateSinWaypoints(double minX, double maxX, double minY, double maxY, 
    double minZ, double maxZ, double stepXY)
  {
    int idx = 0;
    bool isTop = false;
    double z = minZ;
