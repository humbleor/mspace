...
class PatchWorkpp {
 public:
  ...
  // PCL 点云便捷重载：内部转换为 Eigen 后调用原接口
  template <typename PointT>
  void estimateGround(const typename pcl::PointCloud<PointT>::Ptr& cloud_in) {
    Eigen::MatrixXf eigen_cloud(cloud_in->size(), 3);
    for (size_t i = 0; i < cloud_in->size(); i++) {
      eigen_cloud.row(i) << cloud_in->points[i].x, cloud_in->points[i].y, cloud_in->points[i].z;
    }
    estimateGround(eigen_cloud);
  }
