                                           ...
                                           const Eigen::Vector3d &end_acc, const Eigen::VectorXd &Time)
{
  ...
  Eigen::MatrixXd poly_coeff(seg_num, 3 * 6); //多项式为6阶
  Eigen::VectorXd Px(6 * seg_num), Py(6 * seg_num), Pz(6 * seg_num); //对xyz三个方向分别分段
  ...
  for (int k = 0; k < seg_num; k++)
  {
    ...
    Dx(k * 6) = Pos(0, k);  //导数的六项参数的前两项为第k和k+1个点的坐标 0 1
    Dx(k * 6 + 1) = Pos(0, k + 1);
    Dy(k * 6) = Pos(1, k);
    Dy(k * 6 + 1) = Pos(1, k + 1);
    Dz(k * 6) = Pos(2, k);
    ...
    if (k == 0) //如果为起点
    {
      Dx(k * 6 + 2) = start_vel(0); //2 4项为速度 加速度
    ...
    else if (k == seg_num - 1) //如果为终点
    {
      Dx(k * 6 + 3) = end_vel(0);   //3 5项为速度 加速度
      Dy(k * 6 + 3) = end_vel(1);
      Dz(k * 6 + 3) = end_vel(2);
  ...
  num_d = 6 * seg_num;
  Ct = Eigen::MatrixXd::Zero(num_d, num_f + num_p); //num_f + num_p 表示约束项的个数
  Ct(0, 0) = 1;
  Ct(2, 1) = 1;
